...
class Running extends Model
{
    ...
    public function operation($where,$time){
        ...
        $data = Running::query()
            ...
            ->join('admin_repertory as y',function($join){
                $join->on('y.good_id','=','r.good_id')
                    ->on('y.warehouse_id','=','r.warehouse_id');
            }, null,null,'left')
//            ->leftJoin('admin_repertory as y','y.good_id','=','r.good_id')
//            ->leftJoin('admin_repertory as yy','yy.warehouse_id','=','r.warehouse_id')
